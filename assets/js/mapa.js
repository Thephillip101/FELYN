...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Ya no hay ninguna clave escrita aqui: la prueba de que es la correcta es
  // que el contenido se pueda descifrar. Ver assets/js/descifrar.js.
  const CLAVE_VISTAS = "felyn_estrellas_vistas";
  const CLAVE_FINAL = "felyn_final_visto";
  ...
  // Cuánto tarda la pista de ayuda en aparecer al entrar. Va DESPUÉS de la
  // animación de entrada a propósito: los primeros segundos son para mirar el
  // cielo, no para leer instrucciones. Una vez visible se queda hasta que ella
  // toca algo, y vuelve sola si se queda quieta mucho rato.
  const ESPERA_DE_PISTA = 4.5;
  ...
  // Una nota con "desde" no existe hasta esa fecha. Es lo que permite que el
  // universo siga creciendo solo, sin tener que subir nada ese dia: la estrella
  // aparece cuando llega el momento y ya.
  const yaLeToca = (dato) => {
    if (!dato.desde) return true;
    const partes = String(dato.desde).split("-").map(Number);
    if (partes.length !== 3 || partes.some(isNaN)) return true;
    const [anio, mes, dia] = partes;
    // Se compara contra el dia, no contra la hora: aparece al empezar ese dia.
    const hoy = new Date();
    const inicioDeHoy = new Date(hoy.getFullYear(), hoy.getMonth(), hoy.getDate());
    return inicioDeHoy >= new Date(anio, mes - 1, dia);
  };

  // Se llenan cuando la clave abre el contenido, no antes: hasta ese momento
  // las notas todavia estan cifradas y "mensajes" ni siquiera existe.
  let mensajesALaVista = [];
  let CANTIDAD = 0;
  let RADIO_INTERNO = 0;
  let RADIO_EXTERNO = 0;
  let MUNDO_ANCHO = 0;
  ...
  // Queda en true cuando acaba de abrir la ultima estrella que le faltaba: el
  // final se celebra al cerrar ese mensaje, no encima de el.
  let finalPendiente = false;
  ...
  // Si la pista está a la vista. Sirve para no apagarla antes de tiempo: si
  // ella toca la pantalla mientras la pista todavía no ha aparecido, no hay
  // que cancelar su entrada, solo dejar que salga cuando le toca.
  let pistaALaVista = false;
  ...
  // Mientras se esta comprobando una clave no se admite otra: derivar la llave
  // tarda un momento a proposito, y sin esto se podrian encadenar intentos.
  let comprobando = false;

  const handlePasswordEntry = async () => {
    if (comprobando || universoAbierto) return;
    const claveEscrita = passwordInput.value.toUpperCase();

    // Esto va AQUÍ, dentro del gesto, y ANTES de cualquier espera: en iPhone el
    // audio solo se desbloquea si arranca en el mismo toque. Suena en silencio
    // hasta que iniciarMusica() sube el volumen, asi que si la clave estaba mal
    // no se oye nada.
    if (window.prepararMusica) window.prepararMusica();

    comprobando = true;
    const textoBoton = enterButton.textContent;
    enterButton.textContent = "ABRIENDO…";
    const abrio = await window.abrirContenido(claveEscrita);
    enterButton.textContent = textoBoton;
    comprobando = false;
  ...
  // Una sola vez y punto. Sin esta guarda, pulsar Enter dos veces (o Enter y
  // despues el boton) construia el universo DOS veces: el doble de estrellas,
  // dos bucles corriendo y dos naves. Es facilisimo de provocar sin querer.
  let universoAbierto = false;

  const hideSplashScreen = () => {
    if (universoAbierto) return;
    universoAbierto = true;
  ...
  const iniciarUniverso = () => {
    // Ahora si hay contenido: se filtra lo que todavia no le toca y se mide.
    mensajesALaVista = mensajes.filter(yaLeToca);
    CANTIDAD = mensajesALaVista.length + cumpleanos.length;
    ...
    gsap.set(pista, { opacity: 0 });
    gsap.to(pista, {
      opacity: 1,
      duration: 1.2,
      delay: ESPERA_DE_PISTA,
      ease: "power2.out",
      onStart: () => {
        pistaALaVista = true;
      },
  ...
  const repartirCuerpos = () => {
    const lista = [
      ...mensajesALaVista.map((dato) => ({ tipo: "estrella", dato, radio: 45 })),
      ...cumpleanos.map((dato) => ({
        tipo: "planeta",
        dato,
  ...
  const marcarVisitada = (id) => {
    ...
    try {
      ...
      // Modo privado, almacenamiento lleno o cookies bloqueadas: no se podrá
      // recordar, pero eso NO debe impedirle leer. Antes esta línea reventaba
      // sin red y se llevaba por delante el resto de alLlegar(), o sea el
      // mensaje que ella venía a ver.
    }
  };

  // ---------- El momento final ----------
  const hayFinalEscrito = () =>
    typeof final !== "undefined" && final && String(final.texto || "").trim() !== "";

  const finalYaVisto = () => {
    try {
      return localStorage.getItem(CLAVE_FINAL) === "1";
    } catch (e) {
      return false;
    }
  };

  const marcarFinalVisto = () => {
    try {
      localStorage.setItem(CLAVE_FINAL, "1");
    } catch (e) {
      // Sin memoria se podra repetir, pero nunca debe impedir que lo vea.
    }
  };

  const quedanEstrellasPorAbrir = () =>
    cuerpos.some((c) => c.tipo === "estrella" && c.el.classList.contains("nueva"));

  // Todo el cielo destella, desde el centro hacia afuera. Va escalonado a
  // proposito: sesenta y pico de destellos a la vez serian carisimos de pintar
  // y ademas se verian como un parpadeo, no como una ola.
  const celebrarFinal = () => {
    marcarFinalVisto();
    const PASO = 45;
    const estrellas = cuerpos
      .filter((c) => c.tipo === "estrella")
      .sort((a, b) => a.distancia - b.distancia);

    estrellas.forEach((c, i) => {
      setTimeout(() => {
        c.el.classList.add("revelada");
        c.el.addEventListener(
          "animationend",
          () => c.el.classList.remove("revelada"),
          { once: true }
        );
      }, i * PASO);
    });

    setTimeout(() => mostrarMensaje(final.texto, true), estrellas.length * PASO + 800);
  };
  ...
  const crearCuerpos = () => {
    ...
    cuerpos = repartirCuerpos().map((cuerpo) => {
      ...
      if (tipo === "estrella") {
        ...
        // Aro de aviso mientras no lo haya visitado, igual que las estrellas:
        // al volver al universo tiene que verse de un golpe qué le falta.
        // Va como elemento aparte y no como ::after porque en los planetas ese
        // pseudo-elemento ya está ocupado (atmósfera, halo del centro o
        // andamios). El que está en obra no lo lleva: no hay nada que
        // descubrir adentro y sus andamios ya giran solos.
        if (!vistas.includes(dato.id) && !dato.enObra) {
          const aviso = document.createElement("span");
          aviso.className = "planeta-aviso";
  ...
  // Apaga la pista, pero solo si de verdad llegó a verse: así un toque durante
  // los primeros segundos no se lleva por delante su aparición.
  const ocultarPista = () => {
    if (!pistaALaVista) return;
    pistaALaVista = false;
  ...
  const alLlegar = (cuerpo) => {
    if (cuerpo.tipo === "estrella") {
      ...
      // Era la ultima que le faltaba? Se anota para celebrarlo cuando cierre
      // este mensaje: primero que lea, despues la fiesta.
      if (eraNueva && !quedanEstrellasPorAbrir() && hayFinalEscrito() && !finalYaVisto()) {
        finalPendiente = true;
      }
      mostrarMensaje(cuerpo.dato.texto, cuerpo.dato.especial);
    } else {
      // Los planetas también se recuerdan: si no, al volver parecían todos
      // igual de nuevos y no había forma de saber cuáles ya había abierto.
      marcarVisitada(cuerpo.dato.id);
      const aviso = cuerpo.el.querySelector(".planeta-aviso");
