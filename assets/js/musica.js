...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const ponerPista = (autoplay) => {
    // Sin canciones no hay nada que hacer, y sin esta guarda se intentaría leer
    // el archivo de una pista inexistente y reventaría el reproductor entero.
    ...
    if (autoplay) {
      audio.play().catch(() => {
        // El navegador se negó a arrancar solo. No se puede forzar, pero sí
        // avisar: el botón de play se enciende y late para que se vea que la
        // música está ahí esperando. Antes esto se tragaba en silencio y el
        // universo se abría mudo sin que nadie supiera por qué.
        botonPlay.classList.add("pendiente");
      });
    }
  };
  ...
  const actualizarIconoPlay = () => {
    ...
    // Si ya está sonando, el aviso sobra.
    if (!audio.paused) botonPlay.classList.remove("pendiente");
  };
  ...
  // OJO con estas dos funciones: el orden importa y no es un capricho.
  //
  // prepararMusica() tiene que llamarse DENTRO del gesto de ella, o sea en el
  // mismo clic de ENTRAR. En iPhone, Safari solo deja arrancar audio si la
  // llamada a play() sale del propio gesto; medio segundo más tarde, desde el
  // final de una animación, lo rechaza. Así se abría el universo en silencio,
  // y encima sin ningún aviso. En Android no hacía falta, por eso no se notó.
  window.prepararMusica = () => {
    if (preparado) return;
    preparado = true;
    armarLista();
    nuevaVuelta();
    // Arranca en silencio: se oye recién cuando iniciarMusica() sube el volumen.
    audio.volume = 0;
    ponerPista(true);
  };

  // Esta sí va después, cuando el universo ya está a la vista: muestra la barra
  // y sube el volumen despacio.
  window.iniciarMusica = () => {
    if (iniciado) return;
    iniciado = true;
    reproductor.classList.remove("hidden");
    // Por si prepararMusica no llegó a llamarse (no debería pasar).
    window.prepararMusica();
