document.addEventListener("DOMContentLoaded", () => {
  ...
  // La clave ya no va escrita aqui. Este es su huella (SHA-256): sirve para
  // comprobar lo que ella escribe, pero leerla no te dice cual es la clave.
  // Importa porque el universo grande cifra sus notas con esa misma clave: si
  // estuviera escrita en este archivo, cualquiera podria abrir aquello.
  const HUELLA_CLAVE =
    "e252b38fbaff0556362864597df0d5f91acabe656daf197af9e58958248dc199";

  const huellaDe = async (texto) => {
  ...
  // Acá había cuatro efectos de sonido (sfx_click, sfx_message, sfx_unlock y
  // sfx_modal_open), pero esos archivos nunca llegaron a existir en el
  // proyecto: solo daban error 404 y no sonaba nada. Se quitaron para que la
  // página quede limpia. Si algún día aparecen, se vuelven a agregar acá.
  ...
  const handlePasswordEntry = async () => {
    ...
    if ((await huellaDe(enteredPassword)) === HUELLA_CLAVE) {
      hideSplashScreen();
    } else {
      gsap
        .timeline()
  ...
  const toggleAudio = () => {
    if (backgroundAudio.paused) {
      ...
      speakerIcon.innerHTML = `<polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><path d="M19.07 4.93a10 10 0 1 1 0 14.14M22.42 1.58a15 15 0 1 1 0 20.84"></path>`;
    } else {
  ...
  const initGameElements = () => {
    sunflowerBaseImage.src = "../assets/images/sunflower_base.webp";
